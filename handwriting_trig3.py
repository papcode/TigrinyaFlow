...
def create_handwriting_animation_html_filled(text, pen_style="Realistic", writing_style="Natural", animation_speed=3.5):
    """Create HTML5 Canvas-based handwriting animation with filled text"""
...
<body>
    <div class="container">
        ...
        <div class="info-panel">
            <strong>Animation Settings:</strong><br>
            Pen Style: {pen_style} | Writing Style: {writing_style} | Speed: {animation_speed}x | Format: Filled Text
        </div>
    ...
    <script>
        ...
        const config = {{
            ...
            animationFrame: null,
            font: null,
            completedShapes: [] // Store completed character shapes for redrawing
        }};
        ...
        // Convert path commands to fillable segments
        function pathCommandsToSegments(commands, detail = 20) {{
            const segments = [];
            let currentPath = [];
            ...
            for (const command of commands) {{
                if (command.type === 'M') {{
                    ...
                    currentX = command.x;
                    currentY = command.y;
                    currentPath.push({{type: 'M', x: currentX, y: currentY}});
                }} else if (command.type === 'L') {{
                    currentX = command.x;
                    currentY = command.y;
                    currentPath.push({{type: 'L', x: currentX, y: currentY}});
                }} else if (command.type === 'Q') {{
                    // Convert quadratic bezier to small line segments
                    const startX = currentX, startY = currentY;
                    for (let t = 1/detail; t <= 1; t += 1/detail) {{
                        currentX = Math.pow(1 - t, 2) * startX + 2 * (1 - t) * t * command.x1 + Math.pow(t, 2) * command.x;
                        currentY = Math.pow(1 - t, 2) * startY + 2 * (1 - t) * t * command.y1 + Math.pow(t, 2) * command.y;
                        currentPath.push({{type: 'L', x: currentX, y: currentY}});
                    }}
                }} else if (command.type === 'C') {{
                    // Convert cubic bezier to small line segments
                    const startX = currentX, startY = currentY;
                    for (let t = 1/detail; t <= 1; t += 1/detail) {{
                        currentX = Math.pow(1 - t, 3) * startX + 3 * Math.pow(1 - t, 2) * t * command.x1 + 3 * (1 - t) * Math.pow(t, 2) * command.x2 + Math.pow(t, 3) * command.x;
                        currentY = Math.pow(1 - t, 3) * startY + 3 * Math.pow(1 - t, 2) * t * command.y1 + 3 * (1 - t) * Math.pow(t, 2) * command.y2 + Math.pow(t, 3) * command.y;
                        currentPath.push({{type: 'L', x: currentX, y: currentY}});
                    }}
                }} else if (command.type === 'Z') {{
                    if (currentPath.length > 0) {{
        ...
        // Character path generation for filled text
        function generateCharacterPaths(text) {{
            const paths = [];
            const canvas = config.canvas;
            ...
            for (let i = 0; i < text.length; i++) {{
                ...
                const fontPath = config.font.getPath(char, currentX, baseY, fontSize);
                const segments = pathCommandsToSegments(fontPath.commands);
        ...
        // Pen drawing functions (same as before)
        function drawPen(x, y, angle = 0) {{
            const ctx = config.ctx;
            ctx.save();
            ctx.translate(x, y);
        ...
        function animateNextFrame() {{
            ...
            if (currentPath.segments.length === 0) {{
                // Empty character, move to next
                config.currentCharIndex++;
                config.currentSegment = 0;
                config.animationFrame = requestAnimationFrame(animateNextFrame);
                return;
            }}
            
            if (config.currentSegment >= currentPath.segments.length) {{
                // Character complete, store it and move to next
                config.completedShapes.push(currentPath);
                config.currentCharIndex++;
                config.currentSegment = 0;
                config.animationFrame = requestAnimationFrame(animateNextFrame);
                return;
            ...
            const segment = currentPath.segments[config.currentSegment];
            drawSegmentAnimated(segment, currentPath, () => {{
        ...
        // Enhanced segment drawing for filled appearance
        function drawSegmentAnimated(segment, charPath, callback) {{
            ...
            function drawNextPoint() {{
                ...
                // Redraw all completed characters
                redrawCompletedCharacters();
                
                // Draw current character's completed segments
                drawCompletedSegments(charPath, config.currentSegment);
                
                // Draw current segment up to current point
                drawPartialSegment(segment, pointIndex);
                
                // Show pen at current position if we have a valid point
                if (pointIndex < segment.length && segment[pointIndex].x !== undefined) {{
                    drawPen(segment[pointIndex].x, segment[pointIndex].y - 20);
                }}
                
                pointIndex++;
                
                // Capture frame for potential export
                config.frames.push(ctx.getImageData(0, 0, config.canvas.width, config.canvas.height));
                
                setTimeout(drawNextPoint, 80 / config.animationSpeed);
            }}
            
            drawNextPoint();
        }}

        // Draw partial segment (up to a certain point)
        function drawPartialSegment(segment, endIndex) {{
            ...
            // For filled appearance, we'll draw the path progressively
            ctx.beginPath();
            
            for (let i = 0; i <= Math.min(endIndex, segment.length - 1); i++) {{
            ...
            // Fill if the path is closed, otherwise stroke
            if (segment.some(p => p.type === 'Z') && endIndex >= segment.length - 1) {{
                ctx.fill();
            }} else {{
                ctx.stroke();
            }}
        ...
        // Draw completed segments of current character
        function drawCompletedSegments(charPath, upToSegment) {{
            const ctx = config.ctx;
            ctx.fillStyle = '#2c3e50';
            ctx.strokeStyle = '#2c3e50';
            ctx.lineWidth = 2;
            
            for (let i = 0; i < upToSegment; i++) {{
                const segment = charPath.segments[i];
                ...
                // Fill closed paths, stroke open paths
                if (segment.some(p => p.type === 'Z')) {{
                    ctx.fill();
                }} else {{
                    ctx.stroke();
                }}
        ...
        // Redraw all completed characters
        function redrawCompletedCharacters() {{
            const ctx = config.ctx;
            ctx.fillStyle = '#2c3e50';
            ctx.strokeStyle = '#2c3e50';
            ctx.lineWidth = 2;
            
            for (const charPath of config.completedShapes) {{
                for (const segment of charPath.segments) {{
                    if (!segment || segment.length === 0) continue;
